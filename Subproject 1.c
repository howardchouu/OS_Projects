...
int main(int argc, char** argv){
    ...
    //fork 之 前 要 mmap 的部分,因為map到同一個地方,兩個都各自有一份 
    struct Data* data = mmap(NULL, SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);// data是指向memory的指標 PROT_READ--->映射區域可被讀入 PROT_WRITE--->映射區域可被寫入  MAP_SHARED--->分享mapping官方文件
...
//兩個人拿到process pid 父process拿到子process pid 子process拿到0
    ...
    if (pid == 0) {//產生子process
        int my_pid = getpid();//getpid()可以把現在process id傳給你 
        ...
        while(quota){//quota=0--->結束 每猜一次quota減1
            if(!data->flag){//flag==0才能進入
                if(data->row_idx == row_answer && data->col_idx == col_answer){
                    ...
                    write(1, buffer, sdata->flag = -1;//flag設成-1--->猜對(結束) flag--1--->猜錯,繼續
                    break;
                }
                else{
                    ...
                    if(!--quota){//猜的次數用完了
                        ...
                        if(row_answer > data->row_idx){//child process設定正確答案answer parent猜的data->idx
                            data->row_hint = 1;//row值比較小
                            sprintf(buffer + strlen(buffer), " down");
                        }
                        else if(row_answer < data->row_idx){//child process設定正確答案answer parent猜的data->idx
        ...
        (void)waitpid(pid, &status, 0);//等child process結束,parent proceess再結束--->防止zombie process
